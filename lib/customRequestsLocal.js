...
export const CUSTOM_REQUESTS_STORAGE_KEY = "manzili_custom_requests_v1";

export function getLocalCustomRequests() {
  if (typeof window === "undefined") return [];
  try {
    const raw = localStorage.getItem(CUSTOM_REQUESTS_STORAGE_KEY);
    if (!raw) return [];
...
export function setLocalCustomRequests(requests) {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(CUSTOM_REQUESTS_STORAGE_KEY, JSON.stringify(requests));
  } catch (e) {
    console.warn("customRequestsLocal: could not save", e);
  }
}
