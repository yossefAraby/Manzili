...
/** Legacy seeded demo rows (removed on migrate for existing browsers). */
const LEGACY_DEMO_STORE_IDS = new Set(["store_1", "store_2", "store_3"]);
const LEGACY_DEMO_ORDER_IDS = new Set(["order_1", "order_2"]);
...
export const STORAGE_KEYS = {
  ...
  NOTIFICATIONS: "manzili_notifications_v1",
};
...
/** Drop catalog seed stores/orders shipped with early demos (user accounts were never real logins). */
function purgeLegacyDemoCommerce() {
  if (!hasWindow()) return;

  const stores = readStorageItems(STORAGE_KEYS.STORES);
  if (stores.length && stores.every((s) => LEGACY_DEMO_STORE_IDS.has(s.id))) {
    writeStorageEnvelope(STORAGE_KEYS.STORES, []);
  }

  const orders = readStorageItems(STORAGE_KEYS.ORDERS);
  if (orders.length && orders.every((o) => LEGACY_DEMO_ORDER_IDS.has(o.id))) {
    writeStorageEnvelope(STORAGE_KEYS.ORDERS, []);
    writeStorageEnvelope(STORAGE_KEYS.ORDER_ITEMS, []);
  }
}
